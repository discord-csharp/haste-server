...
DocumentHandler.prototype.handlePost = function (request, response) {
  ...
  var onSuccess = function () {
    // check for undesired usage
    var suspicious = false;
    let list = process.env.BLOCKLIST || "";
    let listArr = list.split(";")
    for (let item of listArr) {
      if (buffer.includes(item)) {
        suspicious = true;
        winston.warn('document might be suspicious');
      }
    }
    ...
    _this.chooseKey(function (key) {
      let fileKey = suspicious ? 'suspicious/' + key : key;
      _this.store.set(fileKey, buffer, function (res) {
        if (res) {
          winston.info('added document', { key: key });
          response.writeHead(200, { 'content-type': 'application/json' });
          response.end(JSON.stringify({ key: key }));
        }
        else {
          winston.info('error adding document');
